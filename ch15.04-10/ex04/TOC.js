...
(() => {
  ...
  for (const heading of headings) {
    ...
    const fragmentName = `TOC${sectionNumber.replace(/\./g, '-')}`;
    ...
    link.addEventListener("click", e => {
      e.preventDefault();
      const target = document.querySelector(`#${fragmentName}`);
      if (!target) return;
      /* 以下の2方法、ChromeではOKだったがVivaldiではスムーズな移動ができなかった */
      /* target.scrollIntoView({behavior: 'smooth'}); */
      /* ビューポートの原点からの相対位置: rect.top(or x), rect.left(or y) */
